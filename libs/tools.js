var tools = {
    ...
    */
    cookie : function (key,value,option) {
		//先判断有没有传 value
		if(value === undefined){
			//表示只传了key，获取cookie；声明一个cookie存放获取到的数据（字符串）
			...
			//反之为操作cookie（存储和删除），声明一个str储存拼接好的字符串
			//需要编码
	...
	/*  ajax get 方法
        ...
        @param query <object> url后面请求携带的参数
        @param callback <function> 数据处理成功以后的回调函数
        @param isJson <boolean> 是否是以json格式的数据输出
    */
    ajaxGet : function (url,query,callback,isJson) {
		//如果没有传isjson默认为true，传了就是isjson本身
		isJson = isJson === undefined ? true : isJson;
		//1.创建一个ajax的核心对象
		var ajax = new XMLHttpRequest();
		//2.如果有query，在url后面进行拼接
		if(query){
			url += "?";
		...
		//3.open方法打开连接（请求方式get\post，请求的地址url，是否异步true代表异步）
		ajax.open("GET",url,true);
		//4.send方法，发送请求(因为是get，所以不用发送什么，用null或不填写都可以)
		ajax.send(null);
		//5.监听状态的改变
		ajax.onreadystatechange = function () {
			//判断数据状态值和状态码是否已处理完成
			if(ajax.readyState === 4 && ajax.status === 200){
				//判断是否需要用json格式传输，并利用逻辑短路，进行回调函数
	...
	* @param [query] <object> 请求携带的参数（可有可无）
	*/
